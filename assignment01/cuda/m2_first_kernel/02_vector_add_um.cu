...
int main() {
    ...
    CUDA_CHECK(cudaMallocManaged(&um_a, bytes));
    CUDA_CHECK(cudaMallocManaged(&um_b, bytes));
    CUDA_CHECK(cudaMallocManaged(&um_c, bytes));
    fill_random(um_a, n, 1);
    fill_random(um_b, n, 2);

    // 期望的校验和，host 上先算好，同样不计入计时。
    double want = 0;
    // for (int i = 0; i < n; i++) want += (double)(h_a[i] + h_b[i]);
    for (int i = 0; i < n; i++) want += (double)(um_a[i] + um_b[i]);
    ...
    for (int i = 0; i < n; i++) got += (double)um_c[i];
