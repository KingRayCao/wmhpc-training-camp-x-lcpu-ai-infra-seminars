...
{
    return (1u << 4) | (1u << 7) | (1u << 10) | ((N >> 3) << 17) | ((M >> 4) << 24);
}
                             ...
                             float *gD)
{
    // TODO: 按七步实现。
    int tid = threadIdx.x;
    int warp_id = tid >> 5, lane_id = tid & 31;
    ...
    if (warp_id == 0)
    {
        ...
        asm volatile("tcgen05.alloc.cta_group::1.sync.aligned.shared::cta.b32 [%0], %1;" ::"r"(dst), "r"(64));
    ...
    {
        int m = i / K, k = i % K;
        int saddr = swz128(m, k * 2);
        sA[saddr / 2] = gA[m * K + k];
    }
    for (int i = tid; i < N * K; i += blockDim.x)
    {
        int n = i / K, k = i % K;
        int saddr = swz128(n, k * 2);
        sB[saddr / 2] = gB[n * K + k];
    }
...
#pragma unroll
        ...
        {
            int k0 = round * 16;
            uint64_t descA = make_desc_sm100((uint32_t)__cvta_generic_to_shared(sA) + swz128(0, k0 * 2), 0, K * 2 * 8, 2);
            uint64_t descB = make_desc_sm100((uint32_t)__cvta_generic_to_shared(sB) + swz128(0, k0 * 2), 0, K * 2 * 8, 2);
            uint32_t idesc = make_idesc(M, N);
            uint32_t accum = round == 0 ? 0 : 1; // 第一条不累加,后面累加
    ...
    for (int c = 0; c < N; c += 8)
    {
        uint32_t src = taddr + ((uint32_t)(warp_id * 32) << 16) + c;
        float r[8];
        asm volatile(
