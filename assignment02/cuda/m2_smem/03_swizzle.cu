...
static int swizzle_128B(int row, int colByte) { 
    ...
    int phys_chunk = chunk ^ row;
    return row * 128 + (phys_chunk << 4) + in16;
}
static int swizzle_64B(int row, int colByte) { 
    int chunk = colByte >> 4;
    int in16 = colByte & 0xF;
    int phys_chunk = chunk ^ (row & 0x3);
    return row * 64 + (phys_chunk << 4) + in16;
}
static int swizzle_32B(int row, int colByte) { 
    int chunk = colByte >> 4;
    int in16 = colByte & 0xF;
    int phys_chunk = chunk ^ (row & 0x1);
    return row * 32 + (phys_chunk << 4) + in16;
}
